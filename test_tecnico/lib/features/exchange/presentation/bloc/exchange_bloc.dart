...
const Map<String, String> _cryptoApiIds = {
...
class ExchangeBloc extends Bloc<ExchangeEvent, ExchangeState> {
  ...
  ExchangeBloc({required this.getExchangeRate}) : super(ExchangeState.initial()) {
    
    on<AmountChanged>(_onAmountChanged, transformer: _debounce(const Duration(milliseconds: 350)));
...
Future<void> _onRequested(ExchangeRequested e, Emitter<ExchangeState> emit) async {
...
if (!hasOneCryptoOneFiat || amount <= 0) {
  emit(state.copyWith(loading: false, rate: null, receive: null, error: null));
  return;
}
...
final amountCurrencyId = _toApiId(state.fromCode, isCrypto: state.fromIsCrypto);
...
final resp = await getExchangeRate.call(
  ...
  resp.fold(
    ...
    (data) {
      ...
      final receive = _toDouble(data['receive']) ?? (rate != null ? rate * amount : null);
      final eta = (data['eta']?.toString() ?? state.eta);
      emit(state.copyWith(loading: false, rate: rate, receive: receive, eta: eta, error: null));
    },
  );
}
